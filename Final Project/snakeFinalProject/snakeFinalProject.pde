...
void setup()
{
  ...
  size(400,400);
  PImage img;
  img = loadImage("data/fireplace.jpg");
  image(img, 0, 0);
  textAlign(CENTER);
  
  file = new SoundFile(this, "data/santa_baby.mp3");
  file.play();
...
void draw()
{
  ...
  PImage candycane;
  candycane = loadImage("data/candy.jpg");


  PImage candycane_vert, candycane_hori;
  candycane_vert = loadImage ("data/vertical.jpg");
  candycane_hori = loadImage ("data/horizontal.jpg");
  image(candycane_hori, 0,0,width, 10);
  image(candycane_hori, 0, height-8, width, 10);
  image(candycane_vert, 0, 0, 10,height);
  image (candycane_vert, width-8, 0, 10, 400);
  image (candycane_vert, width-8,0,10,height);
  // fill(255);
  // stroke(255);
  // rect(0,0,width,8);
...
void display()
{
  ...
  PImage candycane;
  candycane = loadImage("data/candy.jpg");
  image(candycane, headx[1], heady[1]);
  stroke(sinecolor(0), sinecolor(0.5), sinecolor(0.3));
  fill(0);
  //fill(0);
  //rect(headx[1],heady[1],8,8);
  //...then erase the back end of the snake.
  //fill(sinecolor(0.1),sinecolor(0.3),sinecolor(0.2));
  fill (sinecolor(0), sinecolor(0.5), sinecolor(0.3));
