...
var melanielines; // this is gonna hold the text file

function preload() {
  // ignore the bullshit error that happens when you do this:
  melanielines = loadStrings('./data/melaniemartinez.txt');
  
}

function setup() {
  console.log("there are " + melanielines.length + " lines!"); // how many lines?

  var bigstring = ""; // the WHOLE BOOK in one HUGE STRING
  // concatenate whole book into one string:
  for (var i = 0; i<melanielines.length; i++)
  {
    bigstring+=melanielines[i]+" ";
  }
  ...
  // write line-by-line:
  saveStrings(chapters, 'melaniemartinezlyrics.txt');
